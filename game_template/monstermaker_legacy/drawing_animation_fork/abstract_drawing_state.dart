...
abstract class AbstractAnimatedDrawingState extends State<AnimatedDrawing> {
  ...
  void onFinishAnimationDefault() {
    if (widget.onFinish != null) {
      widget.onFinish!();
      if (debug?.recordFrames == true) resetFrame(debug!);
    }
  }
  ...
  void evokeOnPaintForNewlyPaintedPaths(int currentPaintedPathIndex) {
    ...
    for (int i = lastPaintedPathIndex + 1; i <= lastPaintedPathIndex + paintedPaths; i++) {
  ...
  PathPainter? buildForegroundPainter() {
    ...
    PathPainterBuilder builder = preparePathPainterBuilder(widget.lineAnimation);
  ...
  PathPainterBuilder preparePathPainterBuilder([LineAnimation? lineAnimation]) {
    PathPainterBuilder builder = PathPainterBuilder(lineAnimation!, scale: widget.scale);
