...
class Drawing extends GetxController {
  ...
  void _setInitialPaintWithRandomColor() {
    final i = Random().nextInt(app_colors.brushColors.length);
    _paint = Paint()
      ..color = app_colors.brushColors[i].withOpacity(0.25)
      ..strokeWidth = 12
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round
      ..style = PaintingStyle.stroke;
      ...
      pathList.forEach((fakePath) {
        ...
        for (int i = 1; i < fakePath.length; i++) {
          tempList.last.lineTo(
              _scaleNumber(inputScale: originalHeight, outputScale: outputHeight, number: fakePath[i].item1),
    ...
    for (var paintList in _superPaints) {
      for (var paint in paintList) {
        paints.add(Paint()
          ...
          ..style = paint.style
          ..strokeWidth =
              _scaleNumber(inputScale: originalHeight, outputScale: outputHeight, number: paint.strokeWidth));
      }
    }
    assert(_assertLengths());
    return paints;
